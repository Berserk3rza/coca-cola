...
function getEmail() {
  ...
  var total = (firstName) + ('.') + (lastName) + ('@') + ('chick-fill-a') + ('.com');
  document.getElementsByName('chick_mail')[0].value = total.toLowerCase();
}
...
$(document).ready(function() {
  ...
  $("#chickForm").validate({
    ignore: ".ignore",
      ...
      messages: {
        ...
        "fsu_number_mix": {
          required: "Please enter a restaurant number"
        },
        "restaurant_owner": {
          required: "Please enter the first and last name of the restaurant operator"
        },
        "fleet_size_vldMV_fy21": {
          required: "Please specify how many vehicles you have in your fleet"
        },
        "first_name": {
        ...
        "operator_timeline_mix": {
          required: "Please select a operator timeline"
        },
        "billing_type_mix": {
          required: "Please select a billing timeline"
        },
        "contact_method": {
          required: "Please specify your preferred method of contact"
        },
        "termsconditionsfw": {
  ...
  $('.chick-form input').bind('keyup blur click change', function () { // fires on every keyup & blur
    if ($('.chick-form').validate().checkForm()) {  // checks form for validity
